...
void tfSetup()
{
    ...
    // Pepare the model.
    model = tflite::GetModel(models_mnist_quant_tflite);
...
{
    // Init GPIO pins
    gpio_set_direction(CALC_PIN, GPIO_MODE_OUTPUT);

    initUart(UART_NUMBER);
    initI2C();
    tfSetup();
    zero_input(input->data.f, totalExpectedDataAmount);
    for (;;)
    {
        pntr = test_array;
        ...
        rdIQS572(0x16,pntr,4);
        set_input(input->data.f, pntr,totalExpectedDataAmount);

        if(gpio_get_level(CALC_PIN)==0){
          doInference();
        }
